...
use crate::network::protocol::{authflow_client_handshake_packet, authflow_client_try_accept_handshake_ack};
...
pub enum ClientCreationError {
    SocketConnectionError { error: std::io::Error },
    Timeout { duration: time::Duration },
    ConnectionRejected { reason: packets::auth::ConnectionResponse },
}
...
impl NetClientSharedState {
    pub fn new(client_id_keys: (box_::PublicKey, box_::SecretKey), server_address: SocketAddr) -> Self {
        Self {
            client_id_keys,
            server_address,
        }
...
impl NetClient {
    pub fn new(cfg: ConfigHandle, address: &SocketAddr) -> Result<Self, ClientCreationError> {
        ...
        let socket = std::net::UdpSocket::bind(bindaddr).map_err(|error| ClientCreationError::SocketConnectionError {error})?;
        socket.connect(address).map_err(|error| ClientCreationError::SocketConnectionError {error})?;
        socket.set_nonblocking(true).map_err(|error| ClientCreationError::SocketConnectionError {error})?;
        // TODO: Save/load identifying keys
        let shared_state = Arc::new(NetClientSharedState::new(
            box_::gen_keypair(),
            *address,
        ));
        ...
        let client_thread = thread::Builder::new()
            ...
            .spawn(move || {
                tokrt.block_on(async move {
                    if let Err(e) = client_netmain(cfg_clone, ccon_tx2, socket, shared_state_copy).await {
...
async fn client_netmain(
    ...
    log::info!("Attempting connection to {:?} from {:?}", shared_state.server_address, socket.local_addr());
    let (hs0pkt, hs0state) = authflow_client_handshake_packet(&shared_state.client_id_keys.0, ClientConnectionType::GameClient).expect("Couldn't encode handshake packet");
        ...
        loop {
            match timeout_at(timeout_t.into(), socket.recv(&mut msgbuf)).await {
                ...
                Ok(bytes_received) => {
                    ...
                    match authflow_client_try_accept_handshake_ack(&hs0state, &msg, (&shared_state.client_id_keys.0, &shared_state.client_id_keys.1)) {
                        Ok(resp) => {
                            hs0resp = Some(resp);
                            break 'hsloop;
                        }
    ...
    drop(hs0pkt);
    drop(hs0state);
    let (connresp, cccstate) = hs0resp.ok_or_else(|| std::io::Error::new(std::io::ErrorKind::TimedOut, "Couldn't connect to server, check log for details"))?;
    if connresp != ConnectionResponse::Accepted {
        log::error!("Server didn't accept our connection: {:?}", connresp);
        return Err(std::io::Error::new(std::io::ErrorKind::ConnectionRefused, "Server didn't accept connection, check log for details"));
    }
    log::info!("Connected to server at {:?}: Name(`{}`), Version({}), ID({})", shared_state.server_address, &cccstate.server_name, &cccstate.server_version_id, bxw_util::sodiumoxide::hex::encode(&cccstate.server_id));
    /*'sockloop: loop {
        tokio::select! {
